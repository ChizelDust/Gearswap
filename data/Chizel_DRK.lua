...
function user_setup()
    ...
    state.IdleMode:options('Normal','Dynamis','Phalanx','DT')
	
	state.WeaponSet = 	M{['description']='Weapon Set','Apocalypse','Anguta','GreatSword',
						'Sword','SwordDW','Club','Axe','AxeDW','DaggerDW','Helheim'}
	...
	send_command('bind ^d input /ma "Drain III" <stnpc>')
    ...
    send_command('bind ^numpad5 input /ws "Shadow of Death" <t>')
    send_command('bind ^numpad4 input /ws "Insurgency" <t>')
    send_command('bind ^numpad3 input /ws "Quietus" <t>')
    send_command('bind ^numpad2 input /ws "Entropy" <t>')
    send_command('bind ^numpad1 input /ws "Catastrophe" <t>')
    send_command('bind ^numpad0 input /ws "Cross Reaper" <t>')
	
    send_command('bind !numpad9 input /ws "Aeolian Edge" <t>')
    send_command('bind !numpad8 input /ws "Steel Cyclone" <t>')
    ...
    send_command('bind !numpad2 input /ws "Herculean Slash" <t>')
...
function init_gear_sets()
    ...
    sets.precast.FC = {
        ...
        ring1="Weatherspoon Ring", 					--5 "Weather. Ring +1", --6(4)
	...
	sets.precast.WS['Insurgency'].pdl = set_combine(sets.precast.WS['Insurgency'], sets.pdl)
	...
	sets.precast.WS['Entropy'] = set_combine(sets.precast.WS, {
		...
		head="Sakpata's Helm",
		body="Nyame Mail",
		back=gear.DRK_INT_WSD,
		waist="Fotia Belt",
		})
		
	sets.precast.WS['Entropy'].pdl = set_combine(sets.precast.WS['Entropy'], sets.pdl)
	
	sets.precast.WS['Shadow of Death'] = sets.precast.WS['Sanguine Blade']
	sets.precast.WS['Shadow of Death'].pdl = sets.precast.WS['Shadow of Death']
	sets.precast.WS['Infernal Scythe'] = sets.precast.WS['Shadow of Death']
	sets.precast.WS['Infernal Scythe'].pdl = sets.precast.WS['Shadow of Death']
    ...
    sets.precast.JA['Last Resort'] = {back=gear.DRK_Idle_Cape}
	...
	sets.midcast['Impact'] = {
		...
		ring1="Stikini Ring +1",
		ring2="Archon Ring",
		back=gear.DRK_Impact_Cape,
		waist="Oneiros Rope",
		} -- OA +178 TP/MP (Impact ~1,304 TP)
	...
	sets.Kiting = {legs="Carmine Cuisses +1", ring2="Defending Ring"}
    ...
    sets.engaged = {
		...
		feet="Flam. Gambieras +2",			-- DA+6		
		neck="Abyssal Beads +1",
		waist="Ioskeha Belt +1",			-- DA+9
		ear1="Schere Earring",				-- DA+6
		ear2="Telos Earring",				-- DA+1
		ring1="Moonlight Ring",
		ring2="Niqmaddu Ring",				
		back=gear.DRK_TP_Cape,				-- DA+10
		}
	
	sets.Accuracy = {
		ammo="Seeth. Bomblet +1",			-- +13
		body="Heath. Cuirass +2",			-- +54
		ring2="Regal Ring",					-- +15
		}
	...
	sets.engaged.SW.Accuracy = set_combine(sets.engaged.SW, sets.Accuracy)
	
	sets.engaged.SB = set_combine(sets.engaged.GA, {
		feet="Sakpata's Leggings",			-- SB1 +10
		ring2="Chirich Ring +1",			-- SB1 +10
		})
	
	sets.engaged.SB.Accuracy = set_combine(sets.engaged.GASB, sets.Accuracy)
    ...
    sets.engaged.SB.Accuracy.DT = sets.engaged.SB.DT
	...
	sets.AxeDW = {main="Dolichenus", sub="Zantetsuken"}
	sets.DaggerDW = {main="Malevolence", sub="Ternion Dagger +1"}
